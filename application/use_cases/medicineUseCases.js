import {
  ...
  export const createMedicineUseCases = () => {
    ...
    const updateMedicine = async (id, updateData) => {
      ...
      // Check if staff can update this medicine
      if (
        updateData.staffId &&
    ...
    const removeMedicine = async (id, staffId) => {
      ...
      // Ensure staff can only delete their own registered medicines
      if (medicine.medicalStaff && medicine.medicalStaff._id.toString() !== staffId) {
        throw createError("Зөвхөн өөрийн бүртгэсэн эмийн мэдээллийг устгах боломжтой!", 403);
      }
