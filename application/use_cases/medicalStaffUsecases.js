import {
  ...
  export const createDiagnosisUseCases = () => {
    ...
    const updateDiagnosis = async (id, updateData) => {
      ...
      if (updateData.staffId && diagnosis.medicalStaff._id.toString() !== updateData.staffId) {
        throw createError("Зөвхөн өөрийн бүртгэсэн оношийг засах боломжтой!", 403);
      }
    ...
    const removeDiagnosis = async (id, staffId) => {
      ...
      if (diagnosis.medicalStaff._id.toString() !== staffId) {
        throw createError("Зөвхөн өөрийн бүртгэсэн оношийг устгах боломжтой!", 403);
      }
...
export const createPrescriptionUseCases = () => {
  ...
  const updatePrescription = async (id, updateData) => {
    ...
    if (updateData.staffId && prescription.prescribedBy._id.toString() !== updateData.staffId) {
...
export const createPrescribedMedUseCases = () => {
  ...
  const updatePrescribedMed = async (id, updateData) => {
    ...
    if (updateData.staffId && prescribedMed.medicalStaff._id.toString() !== updateData.staffId) {
      throw createError("Зөвхөн өөрийн бичсэн эмийн жорыг засах боломжтой!", 403);
    }
  ...
  const removePrescribedMed = async (id, staffId) => {
    ...
    if (prescribedMed.medicalStaff._id.toString() !== staffId) {
      throw createError("Зөвхөн өөрийн бичсэн эмийн жорыг устгах боломжтой!", 403);
    }
    
    await deletePrescribedMedById(id);
    return { success: true, message: "Эмийн жорын мэдээлэл амжилттай устгагдлаа!" };
  };
...
export const createPrescribedGuideUseCases = () => {
  ...
  const updatePrescribedGuide = async (id, updateData) => {
    ...
    if (updateData.staffId && prescribedGuide.medicalStaff._id.toString() !== updateData.staffId) {
      throw createError("Зөвхөн өөрийн бичсэн зөвлөмжийг засах боломжтой!", 403);
    }
    ...
    const updatedPrescribedGuide = await updatePrescribedGuideById(id, updateData);
  ...
  const removePrescribedGuide = async (id, staffId) => {
    ...
    if (prescribedGuide.medicalStaff._id.toString() !== staffId) {
      throw createError("Зөвхөн өөрийн бичсэн зөвлөмжийг устгах боломжтой!", 403);
    }
    
    await deletePrescribedGuideById(id);
    return { success: true, message: "Зөвлөмжийн мэдээлэл амжилттай устгагдлаа!" };
  };
